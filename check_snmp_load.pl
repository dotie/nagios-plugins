...
# Cisco ASA5500x cpu/load - Updated for 9.2.3

my $ciscoasa5500x_cpu_5m = "1.3.6.1.4.1.9.9.109.1.1.1.1.8.1"; # Cisco CPU load (5min %)
my $ciscoasa5500x_cpu_1m = "1.3.6.1.4.1.9.9.109.1.1.1.1.7.1"; # Cisco CPU load (1min %)
my $ciscoasa5500x_cpu_5s = "1.3.6.1.4.1.9.9.109.1.1.1.1.10.1"; # Cisco CPU load (5sec %)
...
# valid values 
my @valid_types = ("stand","netsc","netsl","as400","cisco","cata","c5500","c5500x","nsc","fg","bc","nokia","hp","lp","hpux");
# CPU OID array
...
sub print_usage {
    print "Usage: $0 [-v] -H <host> -C <snmp_community> [-2] | (-l login -x passwd [-X pass -L <authp>,<privp>])  [-p <port>] -w <warn level> -c <crit level> -T=[stand|netsl|netsc|as400|cisco|cata|c5500|c5500x|nsc|fg|bc|nokia|hp|lp|hpux] [-f] [-t <timeout>] [-V]\n";
}
...
-c, --crit=INTEGER | INT,INT,INT
   ...
   3 value check : comma separated level for load or cpu for 1min, 5min, 15min 
-T, --type=stand|netsl|netsc|as400|cisco|cata|c5500|c5500x|bc|nokia|hp|lp
	CPU check : 
		...
		c5500x : Cisco ASA 5500x 9.2+ CPU usage
...
sub check_options {
	...
	if (($o_check_type eq "netsl") || ($o_check_type eq "cisco") || ($o_check_type eq "cata") || 
		($o_check_type eq "c5500") || ($o_check_type eq "c5500x") || ($o_check_type eq "nsc") || ($o_check_type eq "hpux")) {
...
############## Cisco ASA 5500x CPU check ################

if ($o_check_type eq "c5500x") {
my @oidlists = ($ciscoasa5500x_cpu_5m, $ciscoasa5500x_cpu_1m, $ciscoasa5500x_cpu_5s);
my $resultat = (Net::SNMP->VERSION lt 4) ?
          $session->get_request(@oidlists)
        : $session->get_request(-varbindlist => \@oidlists);
...
if (!defined ($$resultat{$ciscoasa5500x_cpu_5s})) {
  print "No CPU information : UNKNOWN\n";
  exit $ERRORS{"UNKNOWN"};
}
...
$load[0]=$$resultat{$ciscoasa5500x_cpu_5s};
$load[1]=$$resultat{$ciscoasa5500x_cpu_1m};
$load[2]=$$resultat{$ciscoasa5500x_cpu_5m};
